...
const DashboardPage: React.FC = () => {
  ...
  const handleSelectLog = useCallback((log: LogEntry) => {
    setSelectedLog(current => {
      if (current && getLogRowId(current) === getLogRowId(log)) {
        return null;
      }
  ...
  const cleanRate = displayedTotalCount > 0 ? (displayedSafeCount / displayedTotalCount) * 100 : 0;
  ...
  const selectedLogId = selectedLog ? getLogRowId(selectedLog) : undefined;

  useEffect(() => {
    if (!selectedLog) return;

    const stillVisible = displayLogs.some(log => getLogRowId(log) === getLogRowId(selectedLog));
    if (!stillVisible) {
      setSelectedLog(null);
    }
  }, [displayLogs, selectedLog]);
        ...
        {!projectId && !projectLoading ? (
          ...
          <div className="dash-body">
            <DashboardSidebar
              ...
              onToggleAnomaliesOnly={() => setShowAnomaliesOnly(current => !current)}
            ...
            <main className="dash-main">
              <section className="dash-page-header ops-panel">
                <div className="dash-page-header__copy">
                  <div className="ops-breadcrumbs">
                    <span>Projects</span>
                    <span>/</span>
                    <span>{currentProject?.name || 'Security Dashboard'}</span>
                  </div>
                  <h1 className="dash-page-header__title">Security Dashboard</h1>
                  <p className="dash-page-header__subtitle">
                    Live SOC-grade monitoring for anomalies, incident clustering, parse drift, and detector health.
                  </p>
                </div>

                <div className="dash-page-header__side">
                  <div className="dash-page-header__chips">
                    ...
                    <span className="ops-chip">{lastUpdate ? `Last update ${lastUpdate.toLocaleTimeString()}` : 'Last update N/A'}</span>
                  </div>
              ...
              <section className="ops-panel ops-panel--chart">
                ...
                <div className="ops-chart__footer ops-chart__footer--triple">
                  <div>
                  ...
                  <div>
                    ...
                    <strong className="ops-text-danger">{formatPercent(threatRate)}</strong>
                    <span> across the current slice</span>
                  </div>
                  <div>
                    <p className="ops-overline">Clean rate</p>
                    <strong className="ops-text-success">{formatPercent(cleanRate)}</strong>
                    <span> across the current slice</span>
                  </div>
                </div>
              </section>
              ...
              <section className="ops-panel">
                <div className="ops-section-head">
                  <div>
                    ...
                    <p className="ops-muted-copy">Click a row to inspect model-level scoring while keeping deep inline details and correction actions available.</p>
                  </div>
                  <div className="ops-inline-stats">
                ...
                {isLoading && !browseResults && !searchResults ? (
                  ...
                  <div className={`dash-log-split ${selectedLog ? 'dash-log-split--detail' : ''}`}>
                    <div className="dash-log-split__table">
                      <LogsTable
                        logs={displayLogs}
                        sourceLogs={logs}
                        focusedIp={focusedIp}
